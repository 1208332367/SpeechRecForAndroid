...
public class IatDemo extends Activity implements OnClickListener {
    ...
    private Python python; // 初始化Python环境
    private PyObject pyObject;//"text"为需要调用的Python文件名
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        python=Python.getInstance(); // 初始化Python环境
        pyObject=python.getModule("api");//"api"为需要调用的Python文件名
        ...
        switch (view.getId()) {
            ...
            case R.id.iat_recognize_stream:
                ...
                try {
                    ...
                    InputStream open = getAssets().open("my_test_audio.wav");
    ...
    private RecognizerListener mRecognizerListener = new RecognizerListener() {
        ...
        public void onResult(RecognizerResult results, boolean isLast) {
            ...
            if (isLast) {
                //TODO 最后的结果，需要调用相应的判定API
                String final_result = mResultText.getText().toString();
                String word = "似";
                String delimeter = "，";
                String pinyin = "si";
                Integer pinyin_num = 4;
                // mResultText.setText(final_result);

                // 调用python
                // PyObject res = pyObject.callAttr("getSingleWordSpeechJudge", PyObject.fromJava(final_result), PyObject.fromJava(word));//"getSingleWordSpeechJudge"为需要调用的函数名
                // PyObject res = pyObject.callAttr("getPhraseSpeechJudge", PyObject.fromJava(final_result), PyObject.fromJava(word), PyObject.fromJava(delimeter));
                PyObject res = pyObject.callAttr("getMultiPinyinSpeechJudge", PyObject.fromJava(final_result), PyObject.fromJava(word), PyObject.fromJava(pinyin), PyObject.fromJava(pinyin_num));
                String python_result = res.toJava(String.class);
                //showTip(python_result);
                mResultText.setText(python_result);
            }
        }
